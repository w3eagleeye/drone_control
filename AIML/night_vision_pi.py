...
from examples.camera.picamera import PiCamera
from examples.camera.picamera import PiRGBArray
...
for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
    ...
    for pic, contour in enumerate(contours):
        ...
        if area > 0 and area < 100:
            ...
            if D > 50 and (abs(A) < 15 or abs(A) > 165):
    ...
    if centers_len > 1:
        for center in centers:
            ...
            if (p1[0] < img_center[0] and p2[0] > img_center[0]) or (p1[0] > img_center[0] and p2[0] < img_center[0]):
                ...
                if D > 100 and (abs(A) < 15 or abs(A) > 165):
